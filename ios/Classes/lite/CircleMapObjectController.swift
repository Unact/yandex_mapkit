...
class CircleMapObjectController: NSObject, MapObjectController, YMKMapObjectTapListener {
  ...
  public required init(
    ...
    let circle = parent.addCircle(with: UtilsLite.circleFromJson(params["circle"] as! [String: Any]))
  ...
  public func update(_ params: [String: Any]) {
    if (!internallyControlled) {
      circle.geometry = UtilsLite.circleFromJson(params["circle"] as! [String: Any])
    ...
    circle.strokeColor = UtilsLite.uiColor(fromInt: (params["strokeColor"] as! NSNumber).int64Value)
    circle.strokeWidth = (params["strokeWidth"] as! NSNumber).floatValue
    circle.fillColor = UtilsLite.uiColor(fromInt: (params["fillColor"] as! NSNumber).int64Value)
